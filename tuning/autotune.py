...
python autotune.py winograd 1286.mlir --Xtune='--lhs-dims=bmk' --Xtune='--rhs-dims=bkn' --Xtune='--tile-dims=*mnk' --devices=1,3,5 --num-candidates=64
...
DEFAULT_MAX_CPU_WORKERS = multiprocessing.cpu_count() # the actual amount of worker that will be generated = max(min(max_cpu_workers, len(task_list)), 1)
...
def parse_devices(devices_str: str) -> list[int]:
    ...
    try:
        ...
        raise argparse.ArgumentTypeError(f"Invalid device list: {devices_str}. Error: {e}")
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument('--devices', type=parse_devices, default=[0], help="Comma-separated list of device IDs (e.g., --devices=0,1). Default: [0]")
    parser.add_argument('--max-cpu-workers', type=int, default=DEFAULT_MAX_CPU_WORKERS, help=f"Max number of workers for CPU-bounding tasks (default: {DEFAULT_MAX_CPU_WORKERS}, the number of CPUs in current system)")
    
    # ./tune.py options
    # TODO: going to invoke tune.py feature directly in future
    parser.add_argument('--Xtune', action="append", default=[], help="Extra options passed to tune.py (e.g., --Xtune='--opt=val' --Xtune='--opt2=val2')")
...
def setup_logging(args: argparse.Namespace, log_dir: Path) -> Path:
    ...
    logging.info(f"Extra options for tune.py: {args.Xtune}")
...
def worker_run_command_with_device_id(args: argparse.Namespace, command: list[str], check: bool = True) -> subprocess.CompletedProcess:
...
def compile_candidates(args: argparse.Namespace, base_dir: Path, candidates: list[Path], candidate_dir: Path) -> tuple[list[Path], Path]:
...
def benchmark_top_candidates(args: argparse.Namespace, base_dir: Path, candidates_dir: Path, compiled_files: list[Path]) -> Path:
    ...
    with multiprocessing.Pool(len(args.devices), init_worker_context, (worker_context_queue,)) as worker_pool:
...
def benchmark_unet(args: argparse.Namespace, base_dir: Path, unet_candidates: list[str]) -> None:
...
def main():
    ...
    compiled_files, compiled_dir = compile_candidates(args, base_dir, candidates, candidates_dir)
