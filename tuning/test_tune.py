import pytest
from tune import (
    Configuration, get_contract_tile_sizes, get_pipeline_config, get_shape_dims,
    get_shapes_mmt, get_shapes_conv, get_shapes_contract, get_shapes_batch_matmul
)
...
def test_get_contract_tile_sizes():
    ...
    assert get_contract_tile_sizes(config, ["m", "n", "k"]) == [4, 8, 16]
    assert get_contract_tile_sizes(config, ["n", "m", "k"]) == [8, 4, 16]
    assert get_contract_tile_sizes(config, ["k", "n", "m"]) == [16, 8, 4]
    assert get_contract_tile_sizes(config, ["k", "k", "k"]) == [16, 16, 16]
...
def test_get_pipeline_config():
    ...
    assert get_pipeline_config(config1) == ""
    assert get_pipeline_config(config2) == ', no_reorder_workgroups, llvm_func_attrs = {"amdgpu-waves-per-eu" = "4"}'

def test_get_shape_dims():
    assert get_shape_dims("2048x1280xf16") == [2048, 1280]
    assert get_shape_dims("64x32x128xf32") == [64, 32, 128]
...
def test_get_shapes_mmt():
    ...
    assert get_shapes_mmt(template) == (2048, 1280, 1280)
...
def test_get_shapes_conv():
    ...
    assert get_shapes_conv(template) == (1, 1, 32, 256, 3, 3, 1280)
...
def test_get_shapes_contract():
    ...
    assert get_shapes_contract(template) == ([2048, 1280], [1280, 1280], [2048, 1280])
...
def test_get_shapes_batch_matmul():
    ...
    assert get_shapes_batch_matmul(template) == ([1, 32, 1024], [1, 1024, 32], [1, 32, 32])